module.exports = function({ api, models }) {
  ...
  const checkttDataPath = __dirname + '/../Sanan/commands/checktt/';
  setInterval(async() => {
...
(async function () {
	...
	//DEFINE DATLICH PATH
	const datlichPath = __dirname + '/../Sanan/commands/cache/datlich.json';
		...
		for (el of temp) {
			...
			if ("ATTACHMENT" in el) {
				...
				for (a of el.ATTACHMENT) {
					...
					fs.writeFileSync(__dirname + `/../Sanan/commands/cache/${a.fileName}`, Buffer.from(getAttachment, 'utf-8'));
					out.attachment.push(fs.createReadStream(__dirname + `/../Sanan/commands/cache/${a.fileName}`));
				}
			}
			console.log(out);
			if ("BOX" in el) await api.setTitle(el["BOX"], el["TID"]);
			api.sendMessage(out, el["TID"], () => ("ATTACHMENT" in el) ? el.ATTACHMENT.forEach(a => fs.unlinkSync(__dirname + `/../Sanan/commands/cache/${a.fileName}`)) : "");
		}
